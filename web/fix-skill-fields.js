...
console.log('🔧 修复技能字段 - 确保新角色与原有格式完全兼容');
console.log('='.repeat(80));
...
const ALL_CHARACTERS = [...WAIFU_CHARACTERS, ...HISTORICAL_FIGURES, ...FICTION_WORLDS];

console.log('\n📝 检查前5个原有技能，确认标准格式:');
publicData.summaries.slice(0, 3).forEach((s, i) => {
  console.log(`  ${i + 1}. ${s.id}:`, Object.keys(s));
});

console.log('\n🔧 修复所有角色字段，确保格式兼容...');
let fixedCount = 0;

ALL_CHARACTERS.forEach(item => {
  const summary = publicData.summaries.find(s => s.id.toLowerCase() === item.id.toLowerCase());
  if (summary) {
    summary.icon = item.icon;
    summary.title = item.name;
    summary.name = item.name;
    summary.description = item.desc;
    summary.category = item.category;
    summary.subcategory = item.subcategory;
    fixedCount++;
  }
  
  const skill = publicData.skills.find(s => s.id.toLowerCase() === item.id.toLowerCase());
  if (skill) {
...
console.log(`✅ 修复了 ${fixedCount} 个角色的字段`);

console.log('\n🎯 最关键修复：修改 SkillCard 组件使用 skill.icon 字段！');

const skillCardPath = path.join(__dirname, 'src', 'components', 'SkillCard.tsx');
let skillCardCode = fs.readFileSync(skillCardPath, 'utf8');

skillCardCode = skillCardCode.replace(
  /const icon = getCategoryIcon\(category\);/,
  `const icon = ('icon' in skill && skill.icon) ? skill.icon : getCategoryIcon(category);`
);

fs.writeFileSync(skillCardPath, skillCardCode, 'utf8');
console.log('✅ SkillCard.tsx 已修改：优先使用 skill.icon');

fs.writeFileSync(publicPath, JSON.stringify(publicData, null, 2), 'utf8');
fs.writeFileSync(srcPath, JSON.stringify(publicData, null, 2), 'utf8');

console.log('\n📊 验证修复结果:');
ALL_CHARACTERS.slice(0, 5).forEach(item => {
  const s = publicData.summaries.find(x => x.id.toLowerCase() === item.id.toLowerCase());
  if (s) {
    console.log(`  ${s.icon} ${s.title} - icon: ${s.icon ? '已设置✅' : '空白❌'}`);
  }
});

console.log(`\n🎉 完成！所有 ${fixedCount} 个角色都有独立图标了！`);
console.log(`👉 刷新浏览器就能看到每个角色独特的 emoji 图标了！`);
