...
console.log('💥 终极修复：同时更新 skills 和 summaries 两个数组！');
console.log('='.repeat(80));
...
console.log(`\n📊 修复前:`);
console.log(`   - skills 数量: ${publicData.skills.length}`);
console.log(`   - summaries 数量: ${publicData.summaries.length}`);
console.log(`   - ❌ 关键问题：前端使用 summaries 数组，之前只更新了 skills！`);

const filterIds = [...WAIFU_CHARACTERS.map(w => w.id), ...HISTORICAL_FIGURES.map(h => h.id), ...FICTION_WORLDS.map(f => f.id)];

const cleanSkills = publicData.skills.filter(s => !filterIds.some(id => s.id.toLowerCase().includes(id)));
const cleanSummaries = publicData.summaries.filter(s => !filterIds.some(id => s.id.toLowerCase().includes(id)));

console.log(`\n📋 清理后原有技能:`);
console.log(`   - skills: ${cleanSkills.length} 个`);
console.log(`   - summaries: ${cleanSummaries.length} 个`);
console.log(`➕ 新增角色:`);
console.log(`   - 情绪化女神: ${WAIFU_CHARACTERS.length} 个`);
console.log(`   - 历史人物: ${HISTORICAL_FIGURES.length} 个`);
console.log(`   - 小说世界: ${FICTION_WORLDS.length} 个`);

let num = cleanSummaries.length + 1;

const createBoth = (item, category, subcategory) => {
  const skill = {
    number: String(num),
    metadata: {
...
WAIFU_CHARACTERS.forEach(item => {
  const { skill, summary } = createBoth(item, 'character', 'anime-waifu');
  cleanSkills.push(skill);
  cleanSummaries.push(summary);
});

HISTORICAL_FIGURES.forEach(item => {
  const { skill, summary } = createBoth(item, 'character', 'history');
  cleanSkills.push(skill);
  cleanSummaries.push(summary);
});

FICTION_WORLDS.forEach(item => {
  const { skill, summary } = createBoth(item, 'fiction', 'xianxia');
  cleanSkills.push(skill);
  cleanSummaries.push(summary);
});
...
console.log(`\n✅ 双数组同步完成！`);
console.log(`\n📊 修复后:`);
console.log(`   - skills 数量: ${cleanSkills.length}`);
console.log(`   - summaries 数量: ${cleanSummaries.length}`);
console.log(`\n📍 public/skills-summary.json: 已更新`);
console.log(`📍 src/skills-summary.json: 已更新`);
console.log(`\n🎉 72个角色同时注入 skills 和 summaries 两个数组！`);
console.log(`\n👉 现在强制刷新浏览器 Ctrl+F5 即可看到所有角色！`);
