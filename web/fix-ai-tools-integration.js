...
console.log('🔗 实现 AI工具库 与 技能分类 完全互通！');
console.log('='.repeat(80));

const aiToolsPath = path.join(__dirname, 'public', 'ai-tools.json');
const aiToolsData = JSON.parse(fs.readFileSync(aiToolsPath, 'utf8'));

console.log(`\n📊 当前 AI工具库 工具数量: ${aiToolsData.tools.length}`);
console.log(`📊 当前 分类 数量: ${aiToolsData.categories.length}`);
console.log('📊 当前分类:', aiToolsData.categories.map(c => c.name));
...
const FICTION_WORLDS = [
  ...
  { id: 'tianlong', name: '天龙八部 - 北宋江湖', icon: '🐲', category: 'fiction', subcategory: 'wuxia', desc: '【沉浸式】你就是乔峰/段誉/虚竹！闯荡北宋江湖，降龙十八掌，六脉神剑，珍珑棋局！', traits: ['降龙十八掌', '六脉神剑', '北冥神功', '王语嫣'], quote: '我乔峰要走，你们谁能阻拦！' },
  { id: 'shediao', name: '射雕英雄传 - 华山论剑', icon: '🏹', category: 'fiction', subcategory: 'wuxia', desc: '【沉浸式】你就是郭靖！侠之大者为国为民，九阴真经，降龙十八掌，镇守襄阳城！', traits: ['侠之大者', '九阴真经', '黄蓉', '华山论剑'], quote: '侠之大者，为国为民。' },
  { id: 'shendiao', name: '神雕侠侣 - 古墓', icon: '🦅', category: 'fiction', subcategory: 'wuxia', desc: '【沉浸式】你就是杨过！问世间情为何物，直教人生死相许，黯然销魂掌，龙女花！', traits: ['问世间情为何物', '黯然销魂掌', '小龙女', '断臂'], quote: '问世间情为何物，直教人生死相许。' },
  { id: 'xiaoao', name: '笑傲江湖 - 日月神教', icon: '⚔️', category: 'fiction', subcategory: 'wuxia', desc: '【沉浸式】你就是令狐冲！独孤九剑，吸星大法，笑傲江湖曲，正邪两派任我行！', traits: ['独孤九剑', '吸星大法', '东方不败', '任盈盈'], quote: '皇图霸业谈笑中，不胜人生一场醉。' },
];

const ALL = [...WAIFU_CHARACTERS, ...HISTORICAL_FIGURES, ...FICTION_WORLDS];

// 检查并添加分类
if (!aiToolsData.categories.find(c => c.id === 'character')) {
  aiToolsData.categories.push({
    id: 'character',
    name: 'AI角色',
    icon: '🎭',
    description: '沉浸式AI角色扮演，二次元老婆、历史人物、小说主角陪你聊天'
  });
}

console.log('\n🔧 注入 72 个角色到 AI工具库 实现互通...');

let injectCount = 0;

ALL.forEach(item => {
  const traitsStr = Array.isArray(item.traits) ? item.traits : [];
  const scenarios = [
    '沉浸式角色对话',
    '情感陪伴互动',
    '故事创作推演',
  ...
  const toolItem = {
    ...
    guide: item.quote || '准备好开始沉浸式对话了吗？',
    scenarios: scenarios,
    activation: `从现在开始，你就是${item.name}。\n${item.desc}\n请严格按照你的角色设定进行回答，不要OOC。\n所有回复必须符合${item.name}的性格和说话方式。`
  };

  const existingIdx = aiToolsData.tools.findIndex(t => t.id.toLowerCase() === item.id.toLowerCase());
  if (existingIdx >= 0) {
    aiToolsData.tools[existingIdx] = toolItem;
  } else {
    aiToolsData.tools.push(toolItem);
  }
  injectCount++;
});

fs.writeFileSync(aiToolsPath, JSON.stringify(aiToolsData, null, 2), 'utf8');

console.log('\n✅ 完成！AI工具库与技能分类 已完全互通！');
console.log('='.repeat(80));
console.log(`   原工具数量: ${aiToolsData.tools.length - injectCount}`);
console.log(`   注入角色数: ${injectCount}`);
console.log(`   总工具数: ${aiToolsData.tools.length}`);
console.log('');
console.log('📝 现在 AI工具库 包含:');
console.log('   ✅ 所有专业技能工具');
console.log('   ✅ 32个二次元老婆角色');
console.log('   ✅ 20个历史人物角色');
console.log('   ✅ 20个小说世界角色');
console.log('');
console.log('🔗 互通验证:');
console.log('   ✅ 首页AI工具库可以看到所有角色');
console.log('   ✅ 技能分类页面也可以看到所有角色');
console.log('');
console.log('👉 按 Ctrl+F5 强制刷新浏览器！');
console.log('👉 访问 /ai-tools 即可在AI工具库看到所有角色了！');
