...
data.skills = data.skills.map(skill => {
  ...
  if (rule && !seenIds.has(rule.idMatch)) {
    seenIds.add(rule.idMatch);
    updatedCount++;
    console.log(`清理并保留原ID [${skill.id}]: ${rule.name}`);
  ...
  if (rule && seenIds.has(rule.idMatch)) {
    console.log(`删除重复技能: ${skill.id} - ${skill.name}`);
    return null;
  }
